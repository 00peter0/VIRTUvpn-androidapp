...
object SecureBrowserUrlPolicy {
    ...
    internal fun isPrivateOrLocalHost(rawHost: String?): Boolean {
        ...
        return parseIpv4Literal(host)?.let { isPrivateIpv4(it) } == true ||
            parseIpv6Literal(host)?.let { isPrivateIpv6(it) } == true
    }
    ...
    private fun isPrivateIpv6(address: InetAddress): Boolean {
        if (address is Inet4Address) {
            val hostAddress = address.hostAddress ?: return false
            return parseIpv4Literal(hostAddress)?.let { isPrivateIpv4(it) } == true
        }
